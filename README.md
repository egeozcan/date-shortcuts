...
// Create a parser. Calculations are relative to the current date and time.
const parser = new DateShortcutParser();

// "Tomorrow at 5:30 PM"
const futureDate = parser.parse('t +1d 5:30pm');
...
Components are combined with spaces. The order is generally flexible, but time must always come at the end.
...
| Today | Sets date to today | `t`, `today`, `now` | `t`, `today +1d` |
| Workday | Adjust to closest workday | `.` (at the very end) | `t+2d.` (Saturday -> Friday) |
...
|---|---|---|
| `HH:mm:ss` | 24-hour format. Seconds are optional. | `17:30`, `09:00:15` |
| `h:mm` am/pm | 12-hour format (English locale). Minutes are optional. | `5:30pm`, `9am`, `11pm` |
| `H` or `h` | Hour only, 24-hour or 12-hour format. | `14` (2 PM), `8pm` |
...
// Today plus one day (tomorrow)
parser.parse('t +1d'); // -> 2024-05-16T00:00:00.000Z

// --- Time Parsing ---
// Tomorrow at 5 PM
parser.parse('t+1d 5pm'); // -> 2024-05-16T17:00:00.000Z

// Today at 23:15
parser.parse('t 23:15'); // -> 2024-05-15T23:15:00.000Z

// Just a time - keeps the current date
...
// --- Workday Adjustment ---
// Add 3 days to today (Wednesday -> Saturday), then adjust to closest workday (Friday)
parser.parse('t+3d.'); // -> 2024-05-17T00:00:00.000Z

// --- Using defaultTime ---
const morningParser = new DateShortcutParser({ fromDate, defaultTime: '09:00' });

// Tomorrow, with default time applied
morningParser.parse('t+1d'); // -> 2024-05-16T09:00:00.000Z

// Tomorrow, with explicit time overriding the default
morningParser.parse('t+1d 1pm'); // -> 2024-05-16T13:00:00.000Z

// --- Weekday Parsing ---
// The second weekday of the current month (May 2024)
parser.parse('2wd'); // -> 2024-05-02T00:00:00.000Z (Thursday)

// The last weekday of the current month
